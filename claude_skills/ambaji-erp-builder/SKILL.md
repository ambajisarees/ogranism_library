...
2. **Keyboard First**: Design for `Ctrl+S`, `Alt+1-9`, `NumPad Enter`. Never hijack `Ctrl+T/W/R/F`.
3. **Instantaneous Context**: Split-pane layout means selecting a registry item populates the detail canvas without navigation. Zero page loads.
4. **Semantic Color**: Use the 12-color categorical chart library for role/type differentiation. Calculate text contrast automatically via luminance.
5. **Progressive Disclosure**: Show summary in the list, expand in the canvas. Use `TissueAccordion` for rarely-needed sections.

## Keyboard Shortcut Rules
- **Safe to override**: `Ctrl+K` (command palette), `/` (search), `Esc` (close/cancel)
- **FORBIDDEN**: `Ctrl+T`, `Ctrl+W`, `Ctrl+N`, `Ctrl+R`, `F5`, `Ctrl+F`
- **ERP Productivity**: `Ctrl+S` (save), `Alt+[1-9]` (tabs), `Enter` on numpad (swift entry)
