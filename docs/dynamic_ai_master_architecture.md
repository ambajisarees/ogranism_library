...
For every active state transition (Tab Switch, View Mode Switch, Filter Update, Loading):
1. **View Mode & Tab Switch**: `AnimatedSwitcher` with 150ms `FadeTransition` / `CrossFade` for instant, non-jarring state updates.
2. **Data Loading State**: Native `shad.Skeleton` placeholders in table/list rows during async fetches.
3. **Popover Triggers**: Native `shad.Popover` slide & fade animations with start-alignment (`bottomLeft` / `topLeft`).
