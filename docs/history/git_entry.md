...
| :--- | :--- | :--- | :--- | :--- |
| **2026-08-06** | 🍏 `[MacBook Workstation]` | **Commit & Push** | `master` (`origin/master`) | Master ERP Architecture Plan & 1-Week Roadmap (`docs/architecture/master_architecture_roadmap.md`), Items & Designs Specification (`items_and_designs_architecture.md`), `sq_QUAL` full audit (`sq_qual_schema_analysis.md`), itemsrno invariant anchor & ghost row filter strategy, and deprecated tables registry (`deprecated_tables_registry.md`). |
| **2026-08-05** | 🍏 `[MacBook Workstation]` | **Commit & Push** | `master` (`origin/master`) | Full-sweep rate remediation for 20 discrepancy batches, `DESPNOS` & `notes` column addition to `sb_cutdet_summary`, Batch 229 note update (cost/pc `₹328.70` @ 31.00), physical CSV audit (247 rows), and Notes form field in `ScrCcForm`. |
