...
class CuttingService {
  ...
  /// Fetches sibling cards for a MULTI_VNO in sb_cutdet, merging true millrec metrics
  Future<List<CuttingCardModel>> getSiblingCards(int multiVno) async {
    try {
      ...
      final List<dynamic> sbList = response as List;
      if (sbList.isEmpty) return [];

      final List<int> reccardNos = sbList
          .map((json) => (json['reccardno'] as num?)?.toInt() ?? 0)
          .where((id) => id > 0)
          .toList();

      Map<int, Map<String, dynamic>> millrecMap = {};
      if (reccardNos.isNotEmpty) {
        final millrecResponse = await _db.client
            .schema('IMMBE2627')
            .from('sq_MILLREC')
            .select('RECCARDNO, RATE, WMTS, RMTS, JOBRATE')
            .inFilter('RECCARDNO', reccardNos);

        for (final row in millrecResponse as List) {
          final recNo = (row['RECCARDNO'] as num?)?.toInt() ?? 0;
          if (recNo > 0) {
            millrecMap[recNo] = Map<String, dynamic>.from(row);
          }
        }
      }

      return sbList.map((json) {
        final row = Map<String, dynamic>.from(json);
        final recNo = (row['reccardno'] as num?)?.toInt() ?? 0;
        final mr = millrecMap[recNo];
        if (mr != null) {
          // Merge true values from sq_MILLREC
          row['RATE'] = mr['RATE'];
          row['WMTS'] = mr['WMTS'];
          row['RMTS'] = mr['RMTS'];
          row['JOBRATE'] = mr['JOBRATE'];
        }
  ...
  /// Fetches resolved dates for the 6 timeline stages using Postgres RPC
  Future<Map<String, String?>> getBatchTimeline(int multiVno) async {
    try {
      final response = await _db.client
          .schema('IMMBE2627')
          .rpc('get_cutting_batch_timeline', params: {'p_multi_vno': multiVno});

      if (response == null) return {};
      final map = Map<String, dynamic>.from(response as Map);
      return {
        'grey_purchase': map['grey_purchase'] as String?,
        'print_program': map['print_program'] as String?,
        'stock_received': map['stock_received'] as String?,
        'batch_cut': map['batch_cut'] as String?,
        'job_issued': map['job_issued'] as String?,
