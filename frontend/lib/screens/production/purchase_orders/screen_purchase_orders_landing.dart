...
class _ScreenPurchaseOrdersLandingState extends State<ScreenPurchaseOrdersLanding> {
  ...
  // Category & Counts State (Default to Finish)
  ...
  Future<void> _fetchHeaders({bool resetOffset = true}) async {
    ...
    try {
      ...
      debugPrint('Error fetching PO headers: $e\n$stack');
      if (mounted) {
        setState(() {
          _isLoadingHeaders = false;
        });
  ...
  Widget build(BuildContext context) {
    ...
    return FocusTraversalGroup(
      ...
      child: Column(
        ...
        children: [
          // ==========================================
          // 1. PAGE HEADER (Title: Purchase Orders + Disabled Add Button)
          // ==========================================
          PageHeader<void>(
                  ...
                  onPressed: () {
                    shad.showOverlay(
                      ...
                      shad.PopoverConfiguration(
                        ...
                        builder: (context) => DabSubmodulePopover<PurchaseOrderCategory>(
                          title: 'Submodule',
            ...
            child: () {
              switch (_contextTabIndex) {
                case 0:
                  return Center(
                    child: Text(
                      'Purchase Orders Dashboard (Analytics & Insights)',
  ...
  Widget _buildOrdersSplitView(shad.ThemeData theme) {
    final listItems = _orders.map((po) {
      final isPending = po.paidStatus.toUpperCase() == 'PENDING' || po.paidStatus.toUpperCase() == 'N';
      return DynamicListItem(
        id: '${po.cno}_${po.vno}_${po.type}',
        topLeading: isPending
            ? const shad.OutlineBadge(child: Text('Pending'))
            : const shad.PrimaryBadge(child: Text('Completed')),
        topTrailing: '${po.orderDate.day}/${po.orderDate.month}',
        title: po.partyName,
        amount: '₹${po.billAmt.toStringAsFixed(0)}',
        subtitle: '${po.orderNo} • ${po.primaryQuality}',
        indexNumber: '${po.vno}',
        rawData: {
          'cno': po.cno,
          'vno': po.vno,
          ...
          'status': po.paidStatus,
          'items': po.items,
        },
      );
    }).toList();

    final currentSelectedItem = _selectedOrder != null && listItems.isNotEmpty
        ? listItems.firstWhere(
            (it) => it.id == '${_selectedOrder!.cno}_${_selectedOrder!.vno}_${_selectedOrder!.type}',
      ...
      children: [
        DynamicList(
          ...
          onItemSelected: (selected) {
            ...
            final po = _orders.firstWhere(
              (o) => '${o.cno}_${o.vno}_${o.type}' == selected.id,
              orElse: () => _orders.first,
            );
            _onSelectOrderCard(po);
          },
          width: 340,
          showHeader: false,
        ...
        DynamicContentPane(
          ...
          title: _selectedOrder != null ? _selectedOrder!.displayOrderNo : 'No Order Selected',
          statusBadge: _selectedOrder != null
              ? (_selectedOrder!.paidStatus.toUpperCase() == 'PENDING' || _selectedOrder!.paidStatus.toUpperCase() == 'N'
                  ? const shad.OutlineBadge(child: Text('PENDING'))
                  : const shad.PrimaryBadge(child: Text('COMPLETED')))
              : null,
            ...
            children: [
              if (_selectedOrder != null) ...[
                Wrap(
                  ...
                  runSpacing: 12,
                  children: [
                    _buildFieldTile(theme, 'Order No', _selectedOrder!.displayOrderNo),
                    _buildFieldTile(theme, 'Internal VNO', '#${_selectedOrder!.vno}'),
                    _buildFieldTile(theme, 'Category', _selectedCategory.label),
                    _buildFieldTile(theme, 'Party / Supplier', _selectedOrder!.partyName),
                    _buildFieldTile(theme, 'Quality', _selectedOrder!.primaryQuality),
                    _buildFieldTile(theme, 'Total Pcs / Mtr', '${_selectedOrder!.totPcs} Pcs / ${_selectedOrder!.totMts} Mtr'),
                    _buildFieldTile(theme, 'Order Amount', '₹${_selectedOrder!.billAmt.toStringAsFixed(2)}'),
                  ],
                ),
              ...
              _selectedOrder != null && _selectedOrder!.items.isNotEmpty
                  ? Column(
                      children: _selectedOrder!.items.map((item) {
                        return Padding(
                          padding: const EdgeInsets.symmetric(vertical: 4),
                          child: Row(
                            children: [
                              Expanded(child: Text(item.qual.isNotEmpty ? item.qual : 'Item #${item.srno}')),
                              Text('${item.pcs} Pcs / ${item.mts} Mtr'),
  ...
  Widget _buildOrdersTable(shad.ThemeData theme) {
    final columns = [
      const DynamicTableColumnSpec(key: 'vno', label: 'Voucher No', width: 110),
      const DynamicTableColumnSpec(key: 'orderNo', label: 'Order No', width: 140),
    ...
    final rows = _orders.map((po) {
      return DynamicTableRowData(
        id: '${po.cno}_${po.vno}_${po.type}',
        voucherNo: '${po.vno}',
        partyName: po.partyName,
        designPattern: po.displayOrderNo,
        quantity: '${po.totPcs} Pcs / ${po.totMts} Mtr',
        amount: '₹${po.billAmt.toStringAsFixed(0)}',
        amountValue: po.billAmt,
        status: po.paidStatus.toUpperCase() == 'PENDING' || po.paidStatus.toUpperCase() == 'N' ? 'PENDING' : 'COMPLETED',
        expandedDetails: 'Order Details: ${po.displayOrderNo} • Supplier: ${po.partyName} • Creator: ${po.creator.isNotEmpty ? po.creator : "Admin"}',
      );
    }).toList();
